...
이번 비교에서는 RandomForest가 가장 좋은 성능을 보였다.
...
> 9축 IMU 시계열을 직접 학습하는 1D CNN과 GRU를 비교했지만, 현재 공개 데이터 규모에서는 engineered feature 기반 RandomForest가 더 안정적이었다. 따라서 최종 baseline은 RandomForest로 유지하고, 현장 데이터가 충분히 확보되면 1D CNN/GRU를 다시 학습 후보로 확장한다.
