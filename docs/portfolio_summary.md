...
모델 입력값은 실제 IMU 기록에서 추출한 다음 feature로 구성했다.
...
안전 관련 AI 프로젝트에서는 “그럴듯한 데이터”보다 데이터 출처와 검증 범위를 명확히 밝히는 것이 더 중요하다는 점을 정리했다. 공개 데이터로 검증 가능한 부분은 실제로 수집하고, 공개 데이터가 없는 부분은 임의 생성하지 않고 수집 계획으로 분리하는 방식이 더 신뢰도 높은 포트폴리오 구조다.
